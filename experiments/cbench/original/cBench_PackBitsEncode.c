...
static int PackBitsEncodeChunk(TIFF * tif, tidata_t bp, tsize_t cc, tsample_t s)
{
    tsize_t rowsize = (tsize_t) tif->tif_data;
    ((rowsize > 0) ? (void) (0) : __assert_fail("rowsize > 0", "tif_packbits.c", 193, __PRETTY_FUNCTION__));
    while ((long) cc > 0) {
...
static int PackBitsDecode(TIFF * tif, tidata_t op, tsize_t occ, tsample_t s)
{
    ...
    tif->tif_rawcc = cc;
    if (occ > 0) {
	TIFFError(tif->tif_name, "PackBitsDecode: Not enough data for scanline %ld", (long) tif->tif_row);
	return (0);
    }
    return (1);
}
